...
def create_tag(name: str = "TestTag") -> Tag:
    tag = Tag(name=name)
    db.session.add(tag)
    db.session.commit()
...
def test_tag_get_authorization(
        app: Flask,
        client: testing.FlaskClient,
        std_headers: dict,
        staff_headers: dict,
...
def test_tag_get_list(
    ...
    with app.app_context():
        # given
        COUNT = 3
        tags = [create_tag(f"tag{i}") for i in range(0, COUNT)]
        api_route = f"{ROUTE}/"
        ...
        result_data = json.loads(response.data)
        expected_data = [tag.to_dict() for tag in tags]
...
def test_tag_get_list_authorization(
        app: Flask,
        client: testing.FlaskClient,
        std_headers: dict,
        staff_headers: dict,
...
def test_tag_post(
    ...
    with app.app_context():
        ...
        result_data = json.loads(response.data)
        result_data_db = Tag.query.filter_by(**data).first().to_dict()
...
def test_tag_post_authorization(
        app: Flask,
        client: testing.FlaskClient,
        std_headers: dict,
        staff_headers: dict,
    ...
    with app.app_context():
        # given
        data_without = {"name": "TestTag"}
        data_std = {"name": "TestTag2"}
        data_staff = {"name": "TestTag3"}
        data_admin = {"name": "TestTag4"}
        api_route = f"{ROUTE}/"
...
def test_tag_post_invalid_payload(
        app: Flask,
        client: testing.FlaskClient,
        admin_headers: dict
):
...
def test_tag_patch(
    ...
    with app.app_context():
        ...
        result_data = json.loads(response.data)
        result_data_db = Tag.query.get(tag.id).to_dict()
        result_data_without_id = result_data.copy()
        del result_data_without_id["id"]
        expected_data = data.copy()
...
def test_tag_patch_authorization(
        app: Flask,
        client: testing.FlaskClient,
        std_headers: dict,
        staff_headers: dict,
    ...
    with app.app_context():
        ...
        data_without = {"name": "NewTestTag"}
        data_std = {"name": "NewTestTag2"}
        data_staff = {"name": "NewTestTag3"}
        data_admin = {"name": "NewTestTag4"}
...
def test_tag_patch_invalid_payload(
        app: Flask,
        client: testing.FlaskClient,
        admin_headers: dict
):
...
def test_tag_delete(
    ...
    with app.app_context():
        # given
        tag = create_tag()
        db_model_count_before = Tag.query.count()
        api_route = f"{ROUTE}/{tag.id}"

        # when
        response = client.delete(api_route, headers=admin_headers)

        db_model_count_after = Tag.query.count()
...
def test_tag_delete_invalid_id(
        app: Flask,
        client: testing.FlaskClient,
        admin_headers: dict
):
...
def test_tag_delete_authorization(
        app: Flask,
        client: testing.FlaskClient,
        std_headers: dict,
        staff_headers: dict,
    ...
    with app.app_context():
        # given
        tag_without = create_tag("TestTag")
        tag_std = create_tag("TestTag2")
        tag_staff = create_tag("TestTag3")
        tag_admin = create_tag("TestTag4")

        api_route_without = f"{ROUTE}/{tag_without.id}"
        api_route_std = f"{ROUTE}/{tag_std.id}"
        api_route_staff = f"{ROUTE}/{tag_staff.id}"
        api_route_admin = f"{ROUTE}/{tag_admin.id}"
        ...
        result_data_db_without = Tag.query.get(tag_without.id)
        result_data_db_std = Tag.query.get(tag_std.id)
        result_data_db_staff = Tag.query.get(tag_staff.id)
        result_data_db_admin = Tag.query.get(tag_admin.id)
